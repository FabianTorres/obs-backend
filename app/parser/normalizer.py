...
class Normalizer:
    ...
    def clean_section(self, raw_text, context_name="General"):
        ...
        for line in consolidated_lines:
            # A. Auditoría Preventiva (Aquí detectamos vectores malos)
    ...
    def _audit_line(self, text, context):
        """
        Analiza la línea cruda buscando errores lógicos que no se deben autocorregir (como ambigüedad).
        """
        # 1. VALIDACIÓN DE VECTORES (NUEVO)
        # Buscamos patrones que parezcan vectores: vx123, Vx.12345, VX 123
        # Regex: \b[vV][xX] seguido opcionalmente de punto/espacio, y luego dígitos
        ...
        for match in matches:
            ...
            # REGLA DE NEGOCIO: Un vector debe tener exactamente 6 dígitos
            if len(digits) != 6:
                # Intentamos adivinar la variable dueña de la línea para mejor contexto
    ...
    def _balance_parentheses(self, text, context):
        ...
        if close_count > open_count:
            ...
            self._add_log("CRITICAL", full_context, 
                          f"ERROR GRAVE: Sobraban {diff} paréntesis de cierre. Se eliminaron automáticamente.")
            return new_text

        # Caso 2: Faltan cierres
        if open_count > close_count:
            diff = open_count - close_count
            new_text = text + (")" * diff)
            
            self._add_log("CRITICAL", full_context, 
                          f"ERROR GRAVE: Faltaban {diff} paréntesis de cierre. Se agregaron al final automáticamente.")
            return new_text
    ...
    def _normalize_formula(self, text, context):
        ...
        # 1. Vectores: vx, VX, vX, Vx -> Vx
        text = re.sub(r'\b[vV][xX]\.?\s*(\d+)', r'Vx\1', text)

        # 2. Códigos: c123, C 123 -> C123
        ...
        # Patrón A (Prefix)
        text = re.sub(
            r'=\s*Si\s*\((.*?)\)\s*=\s*(\d+)\s+(\d+)\s*,\s*Sino', 
            r'= SI(\1; \2; \3)', text, flags=re.IGNORECASE
        )

        # Patrón B (Comma)
        text = re.sub(
            r'=\s*(\d+)\s*,\s*si\s*(.*?)\s*,\s*(\d+)\s*,\s*sino', 
            r'= SI(\2; \1; \3)', text, flags=re.IGNORECASE
        )

        # Patrón C (Semi)
        text = re.sub(
            r'=\s*(\d+)\s*;\s*(.+?)\s+(\d+)\s*;\s*si\s*no', 
            r'= SI(\2; \1; \3)', text, flags=re.IGNORECASE
        )
        ...
        if ",si" in text.lower():
            def replace_piecewise(match):
                ...
                cond1 = match.group(3)
                val2 = match.group(4)
                cond2 = match.group(5) # ¡OJO! Ahora regex tiene 5 grupos
                
                if val1.startswith("(") and val1.count("(") > val1.count(")"):
                    var_guess = prefix.split("=")[0].strip()
                    self._add_log("CRITICAL", f"{context} -> {var_guess}", 
                                  f"ERROR GRAVE: Paréntesis extra detectado al inicio de '{val1}'. Se eliminó automáticamente.")
                    val1 = val1[1:]
